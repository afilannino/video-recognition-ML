...
def create_final_model(model_weights=False, weights_path=None):
    ...
    rgb_lstm = LSTM(1024, return_sequences=False, dropout=0.5, name='rgb_lstm')(rgb_input)
    ...
    merge1 = concatenate([rgb_dropout, flow_dropout])
    merge1_dense = Dense(classes_size, activation='softmax', name='merge1_dense')(merge1)

    model = Model(inputs=[rgb_input, flow_input], outputs=merge1_dense)
